...
/// <summary>
/// Manages the music selection dropdown UI, allowing players to choose background music tracks.
/// Put this on MusicUIRoot prefab so it works in ANY scene that includes the prefab.
/// </summary>
public class MusicDropdownUI : MonoBehaviour
{
    [Header("UI")]
    ...
    private void Start()
    {
        ...
        // Build list once on start (works in any scene)
        BuildList();
    }
    ...
    private void ToggleDropdown()
    {
        ...
        // If opened and empty, rebuild once
        if (open && contentParent != null && contentParent.childCount == 0)
            BuildList();
    }
