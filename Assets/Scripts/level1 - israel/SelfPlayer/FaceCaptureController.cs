...
public class FaceCaptureController : MonoBehaviour
{
    ...
    [Header("Simple Crop - No AI")]
    [SerializeField, Range(0.3f, 1f)]
    private float centerCropSize = 0.75f;

    [SerializeField]
    private string introText = "צלם 3 תמונות: חייך, כועס, כועס מאוד";

    [SerializeField]
    private float firstStageDelay = 3f;
    ...
    public void CaptureCurrentStage()
    {
        ...
        if (stage == 0)
            ...
            capturedFurious = snap;

        stage++;

        if (stage >= 3)
        {
            Sprite happy = ToCenteredSprite(capturedHappy);
            Sprite angry = ToCenteredSprite(capturedAngry);
            Sprite furious = ToCenteredSprite(capturedFurious);
    ...
    private void UpdateInstruction()
    {
        ...
        if (stage == 0)
            instructionText.text = "חייך";
        else if (stage == 1)
            instructionText.text = "כועס";
        else
            instructionText.text = "כועס מאוד";
    }

    private Sprite ToCenteredSprite(Texture2D tex)
    {
        if (tex == null)
            return null;

        float size = Mathf.Min(tex.width, tex.height) * centerCropSize;

        float x = (tex.width - size) * 0.5f;
        float y = (tex.height - size) * 0.5f;

        Rect pixelRect = new Rect(x, y, size, size);
