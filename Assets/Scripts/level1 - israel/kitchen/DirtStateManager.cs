...
/// Manages the dirtiness state of the kitchen, including gradual dirt stages and cleaning.
/// Every N clicks increases the dirt stage, up to 4 stages total: 0..3.
/// Dirt accumulation can be disabled from the runtime control panel.
/// </summary>
public class DirtStateManager : MonoBehaviour
{
    public static DirtStateManager Instance { get; private set; }
    ...
    [Header("Overlay UI")]
    [SerializeField] private GameObject dirtOverlayObject; // UI image object overlay
    [SerializeField] private SpriteRenderer dirtOverlayRenderer; // Optional: SpriteRenderer if overlay is in world
    ...
    /// <summary>
    /// Registers a click on a messy item, potentially increasing dirt stage.
    /// If dirt is disabled from the control panel, no dirt is accumulated.
    /// </summary>
    public void RegisterMessyClick()
    {
        if (!ControlPanelUI.DirtEnabled)
            return;

        if (clicksPerStage <= 0)
        {
            Debug.LogWarning("[DirtStateManager] clicksPerStage must be greater than 0.");
            return;
        }
        ...
        int newStage = Mathf.Clamp(clickCount / clicksPerStage, 0, 3);
    ...
    /// <summary>
    /// Applies visual changes based on the current dirt stage.
    /// </summary>
    private void ApplyVisuals()
    {
        bool shouldShowDirt = IsDirty;

        if (dirtOverlayObject != null)
            dirtOverlayObject.SetActive(shouldShowDirt);
    ...
    private void TryTriggerTutorialDirtyStep()
    {
        if (!triggerTutorialOnFirstDirtStage)
            return;

        if (tutorialTriggeredThisDirty)
            return;

        TutorialManager tutorial = tutorialOverride != null ? tutorialOverride : TutorialManager.Instance;

        if (tutorial == null)
            return;

        tutorialTriggeredThisDirty = true;
        tutorial.TriggerDirtTutorial();
    }
}
