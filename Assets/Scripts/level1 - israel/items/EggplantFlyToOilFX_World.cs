...
/// <summary>
/// Spawns an eggplant sprite at spawnPoint and flies it in an arc into targetPoint, usually the boiling oil.
/// World-space version using SpriteRenderer.
/// Also plays a whoosh sound when the eggplant starts flying.
/// </summary>
public class EggplantFlyToOilFX_World : MonoBehaviour
{
    [Header("References")]
    [SerializeField] private Transform spawnPoint;     // Eggplant pile / start point
    [SerializeField] private Transform targetPoint;    // Fry zone / boiling oil
    [SerializeField] private Sprite eggplantSprite;    // Eggplant sprite for the flying FX
    [SerializeField] private Transform worldParent;    // Optional parent, for example Items
    ...
    [Header("Motion")]
    [SerializeField] private float duration = 0.55f;
    [SerializeField] private Vector2 arcOffset = new Vector2(1.2f, 1.4f); // World units
    [SerializeField] private float spinDegrees = -540f;
    ...
    public void Play()
    {
        if (spawnPoint == null || targetPoint == null || eggplantSprite == null)
        {
            Debug.LogWarning("[EggplantFlyToOilFX_World] Missing spawnPoint, targetPoint, or eggplantSprite.");
            return;
        }
        ...
        if (worldParent != null)
        {
            go.transform.SetParent(worldParent, true);
        }
    ...
    private void PlayWhooshSound()
    {
        ...
        {
            whooshAudioSource.PlayOneShot(whooshClip, whooshVolume);
            return;
        }

        AudioSource.PlayClipAtPoint(whooshClip, spawnPoint.position, whooshVolume);
    }
