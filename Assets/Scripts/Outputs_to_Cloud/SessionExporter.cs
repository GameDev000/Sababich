...
{
    const string DashboardFolder = "Dashboard";
    const string HtmlFileName    = "sababich_dashboard.html";
    const string DataMarker      = "const SABABICH_DATA =";
    const string DataEndMarker   = "};";
    ...
    static List<SessionRecord> ExtractSessions(string html)
    {
        try
        {
            int start = html.IndexOf(DataMarker);
            if (start < 0) return new List<SessionRecord>();

            int jsonStart = html.IndexOf('{', start);
            int jsonEnd   = html.IndexOf(DataEndMarker, jsonStart);
            if (jsonStart < 0 || jsonEnd < 0) return new List<SessionRecord>();
    ...
    static string EmbedData(string html, string json)
    {
        int start = html.IndexOf(DataMarker);
        if (start < 0)
        {
    ...
    static void CopyAsset(string fileName, string destDir)
    {
    ...
    static string BuildMinimalShell()
    {
