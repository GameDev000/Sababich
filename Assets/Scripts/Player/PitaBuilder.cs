...
//     {
//         eggplantLayer.enabled = false;
//         eggLayer.enabled = false;
//         saladLayer.enabled = false;
//     }
...
public class PitaBuilder : MonoBehaviour
{
    ...
    public void BuildFromSelection(List<string> ingredients)
    {
        ...
        // אם עדיין אין פיתה – אין פיתה קדמית ואין שכבות
        if (!hasPita)
        {
            if (frontPita != null)
                frontPita.gameObject.SetActive(false);
        ...
        // יש פיתה → מפעילים את האובייקט של הפיתה הקדמית
        if (frontPita != null)
